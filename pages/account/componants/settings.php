...
$tplSettings->setFile('settings', './account/componants/settings.html');
...
if (DATA::isPost('update')) {
    ...
    */
    ...
    // update PHProject account
    $ppFile = "./classes/phproject";
    if(is_dir($ppFile)) {
      pupa::mod($resultAccount->Pseudo, DATA::getPost('email'), DATA::getPost('firstname').' '.DATA::getPost('lastname'), "");
    }
    ...
    if (mysqli_num_rows($req) > 0) {
        if (DATA::getPost('newPassword') === DATA::getPost('newPasswordConfirm')) {
            ...
            */
            ...
            $pcFile = "./classes/php-calendar";
            if(is_dir($pcFile)) {
                cupa::mod($resultAccount->Pseudo, DATA::getPost('newPassword'));
            }
            // update PHProject account password
            $ppFile = "./classes/phproject";
            if(is_dir($ppFile)) {
                pupa::mod($resultAccount->Pseudo, DATA::getPost('email'), DATA::getPost('firstname').' '.DATA::getPost('lastname'), DATA::getPost('newPassword'));
            }
