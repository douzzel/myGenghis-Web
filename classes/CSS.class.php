    ...
    private function normalize_int($size)
    {
        if (is_string($size)) {
            switch (substr($size, -1)) {
                case 'M': case 'm': return preg_replace('`[^0-9]`', '', $size) * 1048576;
                case 'K': case 'k': return preg_replace('`[^0-9]`', '', $size) * 1024;
                case 'G': case 'g': return preg_replace('`[^0-9]`', '', $size) * 1073741824;
            }
        }
        return (int) $size;
    }
